		...
		<div class="tm-main uk-width-medium-3-4 uk-margin-top uk-margin-bottom">
			<div class="uk-clearfix">
    			...
    			<div class="uk-float-right">
    				<a href="<?php echo base_url('/admin/major/edit');?>" class="uk-button uk-button-success"><i class="uk-icon-plus"></i> เพิ่ม</a>
    			</div>
			</div>
			<hr/>
            <form action="<?php echo base_url('admin/major');?>" method="post" name="adminForm">
            	<table class="uk-table">
            		<tr>
            			<td width="100%">
            				Filter:
            				<input type="text" name="major_filter_search" id="search" value="<?php echo set_value('major_filter_search');?>" class="text_area" onchange="document.adminForm.submit();" />
            			...
            			<td nowrap="nowrap">
            				<?php echo $this->helper_lib->getStatusHtml('major_filter_status');?>
            			</td>
            		</tr>
            	</table>
            	...
            	<table class="uk-table" cellpadding="1">
            		...
            		if(count( $items )<=0){
            			...
            			for ($i=0, $n=count( $items ); $i < $n; $i++)
            			{
            				...
            				$link_edit = base_url('admin/major/edit/?id='.$row->id.'&per_page='.$per_page);
            				$link_restore = base_url('admin/major/restore/?id='.$row->id.'&per_page='.$per_page);
            				$link_trash = base_url('admin/major/trash/?id='.$row->id.'&per_page='.$per_page);
            				$link_delete = base_url('admin/major/delete/?id='.$row->id.'&per_page='.$per_page);
            				
            				$status_link = base_url('admin/major/unpublish/?id='.$row->id.'&per_page='.$per_page);
            				if($row->status==0){
            				    $status_link = base_url('admin/major/publish/?id='.$row->id.'&per_page='.$per_page);
            				}
            			?>
            			<tr class="<?php echo "row$k"; ?>">
            				...
            				<td>
            					<a href="<?php echo $link_edit;?>"><?php echo $row->major_eng; ?></a>
            				</td>
            				<td>
            				...
            				<td>
            					...
            					<?php 
            					if($this->helper_lib->getFilterStatus('major_filter_status')=='trash'){
            					?>
