		...
		<div class="tm-main uk-width-medium-3-4 uk-margin-top uk-margin-bottom">
            ...
            <form action="<?php echo base_url('headdepartment/homeroom');?>" method="post" name="adminForm">
                ...
                foreach ($items as $item) {
                    $rowHeadDepartmentAction = $this->homeroom_lib->getHomeroomAction($item->id);
                    $userHeadDepartmentType = 'headdepartment';
                    $actionHeadDepartmentStatus = '';
                    if (isset($rowHeadDepartmentAction)) {
                        $userHeadDepartmentType = $rowHeadDepartmentAction->user_type;
                        $actionHeadDepartmentStatus = $rowHeadDepartmentAction->action_status;
                    } ?>
            	<div class="uk-panel uk-panel-box uk-panel-box-default uk-margin-top">
                    <h3 class="uk-panel-title">
						ภาคการเรียน: <?php echo $item->semester_name.' สัปดาห์ที่: '.$item->week; ?> 
						(<?php echo date_format(date_create($item->join_start), 'Y-m-d').' - '.date_format(date_create($item->join_end), 'Y-m-d'); ?>)
						<?php
                        if ($actionHeadDepartmentStatus!='confirmed') {
                            ?>
						<a class="uk-button uk-button-primary uk-button-small" href="<?php echo base_url('headdepartment/homeroom/approve/?homeroom_id='.$item->id); ?>"><i class="uk-icon-send"></i> กดยืนยันการส่งข้อมูล</a>
						<?php
                        } else { ?>
						<a class="uk-button uk-button-danger uk-button-small" href="<?php echo base_url('headdepartment/homeroom/unapprove/?homeroom_id='.$item->id); ?>"><i class="uk-icon-remove"></i> ยกเลิกการส่งข้อมูล</a>
						<?php } ?>
					</h3>
                	<hr/>
                	<table class="uk-table uk-table-hover" cellpadding="1">
                        ...
                        if (count($items)<=0) {
                            ...
                            for ($i=0, $n=count($advisor_items); $i < $n; $i++) {
                                ...
                                $link_confirm = base_url('headdepartment/homeroom/confirm/?id='.$homeroom_id.'&advisor_id='.$advisor_id); ?>
							<tr class="<?php echo "row$k"; ?>">
                                ...
                                if ($actionStatus=='confirmed') {
                                    ?>
								<a class="uk-button uk-button-danger uk-button-small" href="<?php echo base_url('headdepartment/homeroom/remove_confirm/?homeroom_id='.$item->id.'&advisor_id='.$row->id); ?>"><i class="uk-icon-remove"></i> ยกเลิก</a>
								<?php
                                } ?>
								</td>
								<td>
								<?php
                                echo $this->homeroom_lib->getActionStatusHtml($userHeadDepartmentType, $actionHeadDepartmentStatus); ?>
								</td>
							</tr>
						<?php
                            $k = 1 - $k;
