<div class="uk-container uk-container-center">
	<div class="uk-grid">
		...
		<div class="tm-main uk-width-medium-3-4 uk-margin-top uk-margin-bottom">
			...
			<hr/>
            <form action="<?php echo base_url('headadvisor/advisorgroup');?>" method="post" name="adminForm">
            	...
            	<table class="uk-table uk-table-hover uk-table-responsive uk-table-striped" cellpadding="1">
                    ...
                    if (count($items)<=0) {
                        ...
                        for ($i=0, $n=count($items); $i < $n; $i++) {
                            ...
                            $link_edit = base_url('headadvisor/advisorgroup/edit/?id='.$row->id.'&per_page='.$per_page);
                            $link_restore = base_url('headadvisor/advisorgroup/restore/?id='.$row->id.'&per_page='.$per_page);
                            $link_trash = base_url('headadvisor/advisorgroup/trash/?id='.$row->id.'&per_page='.$per_page);
                            $link_delete = base_url('headadvisor/advisorgroup/delete/?id='.$row->id.'&per_page='.$per_page);
                            
                            $status_link = base_url('headadvisor/advisorgroup/unpublish/?id='.$row->id.'&per_page='.$per_page);
                            if ($row->status==0) {
                                $status_link = base_url('headadvisor/advisorgroup/publish/?id='.$row->id.'&per_page='.$per_page);
                            } ?>
            			<tr>
            				<td>
